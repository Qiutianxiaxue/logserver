...
import { LogData, LogStats } from '../types';
...
export class ResponseFormatter {
  ...
  static formatLogData(log: LogData): LogData {
    return {
      ...
      created_date: log.created_date ? DateTime.toDateString(log.created_date) : undefined
    };
  }
   ...
   * 格式化时间范围显示
   */
  static formatTimeRange(timeRange: { start: string; end: string }): { start: string; end: string } {
    return {
